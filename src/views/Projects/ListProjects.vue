<template>
  <v-container class="max-w-container mx-auto pa-0 rounded-lg">
    <m6-list
      v-if="!FinancialViews"
    ...
    >
      ...
      <template v-slot:after:search>
        <v-col
          id="step4"
          class="align-center mx-2 shrink"
        >
          <list-filtering
        ...
        <template v-slot:default="props">
          <v-row class="d-flex flex-wrap ma-0 w-full">
            ...
            >
              <v-card
                v-intro-if="index === 0"
                class="card-custom-shadow pointer white card-grid"
              ...
              >
               <v-checkbox
                ...
                <v-tooltip bottom>
                  <template v-slot:activator="{ on }">
                    <v-btn
                      v-show="!showFinancials && showDeleteIconApplication === index"
                      absolute
                      class="deleteBtn"
                      color="red lighten-1"
                      dark
                ...
                <v-tooltip bottom>
                  <template v-slot:activator="{ on }">
                    <v-btn
                      ...
                      small
                      v-on="on"
                      @click.stop="goToProject(item, true)"
                    >
                      <v-icon size="20">
                        mdi-folder-plus
                      </v-icon>
                ...
                <div class="align-center d-flex flex-column pb-2 pt-1">
                  ...
                  <v-row
                    no-gutters
                    class="align-center card-bottom"
                  >
                    <v-col cols="6" class="grey--text py-1 text--darken-3 text-caption">
                      {{ $t("cpm.budgetStatus") }}
                    </v-col>
                    <v-col cols="6" class="t--darken-3 text-caption">
                      {{ $t("general.startDate") }}
                    </v-col>
                    <v-col cols="6">
                      <v-card
                        class="py-2 text-center w-half white--text full-width rounded-0 no-rounded"
                      ...
                      <v-col cols="6">
                        <v-card
                          class="py-2 text-center w-half white--text full-width"
          ...
          >
            ...
            >
              <tr>
                ...
                <template v-if="!isPlanned">
                  <td class="maxWidth9rem text-truncate">
                    {{ item.description }}
                  </td>
    ...
    <div v-if="FinancialViews">
      <financial-views @close="setDefaultView"></financial-views>
    </div>
    ...
    >
      ...
      <span
        ...
        @click="goToProposal"
      >Click here</span>
      to go to the Proposals List
    </m6-info>
    <financial-panels
      v-if="showFinancials && !FinancialViews"
...
import FinancialPanels from './Financials/FinancialPanels'

import mixins from '@/modules/cpm/_mixins/index'
import FinancialViews from './Financials/FinancialViews'
...
export default {
  ...
  computed: {
    ...
    isListView() {
      return this.view.val === 'view_list'
    },
    isCommitmentView() {
      return this.view.val === 'commitment'
    },
    isSpendingView() {
      return this.view.val === 'spending'
    },

    FinancialViews() {
      return !(!this.isCommitmentView && !this.isSpendingView)
    },
  ...
  watch: {
    ...
    'pagination.itemsPerPage': async function () {
      if (this.isGridView) {
    ...
    selectedProjectId(v) {
      if (v) {
        this.$vuetify.goTo('#financial-panels', this.scrollOptions)
      }
    }
  },
  ...
  methods: {
    ...
    async setDataInLineItems() {
      const projectsSnap = await db.collection('cpm_projects').where('company_nid', '==', '809').get()
      await Promise.all(projectsSnap.docs.map(async projectDoc => {
        const spendingSnap = await projectDoc.ref.collection('spendings').get()

        if (!spendingSnap.empty) {
          await Promise.all(spendingSnap.docs.map(async doc => {
            await doc.ref.update({
              company_nid: '809'
            })
            // const lineItemsSnap = await doc.ref.collection('line_items').get()
            // 
            // if (!lineItemsSnap.empty) {
            //   await Promise.all(lineItemsSnap.docs.map(async doc => {
            //     const description2 = doc.data().description2
            //     await doc.ref.update({
            //       isCommitment: true,
            //       company_nid: '809',
    ...
    async setSearchTermsArrayInProjects () {
      const projectsSnap = await db.collection('cpm_projects').where('company_nid', '==', '809').get()
      await Promise.all(projectsSnap.docs.map(async projectDoc => {
        const project = {
          ...projectDoc.data(),
        ...
        const searchTerms = (project.title || '').toLowerCase().split(' ')
        searchTerms.push(project.number)

        project.ref.update({searchTerms})
      }))
    },
    ...
    async deleteProject(project) {
      ...
      if (!rfpDocs.empty) {
        ...
        this.deleteProjectMessage =
          '<strong>You have RFPs associated with this project.</strong> Are you sure you want to delete this project?'
        ...
        db.collection('m6user')
          ...
          .then(document => {
            ...
            this.search = this.$h.dg(
              user,
              this.isPlanned
                ? 'optionsFilter.searchPlannedList'
                : 'optionsFilter.searchProjectList',
              this.search
            )
    ...
    getAvatar(teamMember) {
      ...
      if (teamMember && this.images[teamMember.value]) {
        image = this.images[teamMember.value].userImage
      }
      ...
      this.axios
        ...
        .then(response => {
          ...
          usersId.forEach(userId => {
            if (results[userId]) {
              data[userId] = { userImage: results[userId] }
            } else {
              data[userId] = {
