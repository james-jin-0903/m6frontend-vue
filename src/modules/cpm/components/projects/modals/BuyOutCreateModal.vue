      ...
      <v-card class="contact-edit-modal rounded-lg">
        ...
        >
          <v-tab :class="errors.changeInformationTab ? 'red' : ''">
            {{ $t('cpm.projects.ChangeBuyOutInfo') }}
          </v-tab>
          <v-tab :class="errors.changeInformationTab ? 'red' : ''">
            Vendor Information
          </v-tab>
              ...
              <div v-show="tab === 0">
                ...
                >
                  ...
                  >
                    <div class="elevation-2 white">
                      <v-text-field
                        v-model="change.facility"
                        color="primary"
                        hide-details
                        :label="$t('cpm.projects.facility')"
                        outline
                      />
                    </div>
                  </v-col>
                    ...
                    >
                      <div class="elevation-2">
                        <v-select
                          ...
                          :label="$t('itApplications.supportEditModal.priority')"
                          outline
                        />
                      </div>
                    </ValidationProvider>
                  ...
                  >
                    <ValidationProvider
                      v-slot="{ errors }"
                      name="Ordering Department Number"
                      rules="required"
                    >
                      <div class="elevation-2 white">
                        <v-text-field
                          v-model="change.orderingDepartmentNumber"
                          color="primary"
                          :error-messages="errors"
                          hide-details
                          label="Ordering Department Number"
                          outline
                          @update:error="errorTitle"
                        />
                      </div>
                  ...
                  >
                    <ValidationProvider
                      v-slot="{ errors }"
                      name="Charge to Dept/Activity:"
                      rules="required"
                    >
                      <div class="elevation-2 white">
                        <v-text-field
                          v-model="change.chargeTo"
                          color="primary"
                          :error-messages="errors"
                          hide-details
                          label="Charge to Dept/Activity"
                          outline
                          @update:error="errorTitle"
                        />
                      </div>
                  ...
                  >
                    ...
                    >
                      <template v-slot:activator="{ on }">
                        <div class="elevation-2 white">
                          <v-text-field
                            v-model="change.materialRequiredDateText"
                            append-icon="calendar_today"
                            color="blue"
                            hide-details
                            label="Material Required Date"
                            mask="date"
                            outline
                            v-on="on"
                          />
                  ...
                  >
                    <ValidationProvider
                      v-slot="{ errors }"
                      name="Ordering Department Name:"
                      rules="required"
                    >
                      <div class="elevation-2 white">
                        <v-text-field
                          v-model="change.orderingDepartmentName"
                          color="primary"
                          :error-messages="errors"
                          hide-details
                          label="Ordering Department Name"
                          outline
                          @update:error="errorTitle"
                        />
                      </div>
...
export default {
  ...
  data() {
    return {
      ...
      materialRequiredDateMenu: false,
      authorizers: [],
      approvalTypes: [],
      projectId: '',
      tab: 0,
  ...
  computed: {
    ...
    materialRequiredDate: {
      get: function () {
        if (this.$h.dg(this.change, 'materialRequiredDate')) {
          const date = new Date(this.change.materialRequiredDate)
          return `${date.getFullYear()}-${date.getMonth() +
            1}-${date.getDate() + 1}`
        }
      ...
      set: function (newValue) {
        if (newValue) {
          const [year, month, day] = newValue.split('-')
          this.change.materialRequiredDate = new Date(
            `${month}/${day}/${year}`
          ).getTime()
        } else {
          this.change.materialRequiredDate = ''
        }
      }
    },
    preparedDate: {
  ...
  watch: {
    ...
    project() {
      if (!this.isCreate && this.changeId) {
        ...
        const buyOutNumber = this.project.nextBuyOutNumber || 1

        let idString = '00000' + buyOutNumber
        idString = idString.substr(idString.length - 5)
        const year = new Date().getFullYear()

        this.change.number = `BUYOUT-${idString}-${year}`
  ...
  methods: {
    ...
    async save() {
      ...
      if (this.change.materialRequiredDateText) {
        const startDate = DateTime.fromFormat(this.change.materialRequiredDateText, 'MMddyyyy')
        if (!startDate.isValid) {
          this.$snotify.error(
            this.$t('cpm.projects.dateInvalid'),
            this.$t('alerts.error')
          ...
          this.change.materialRequiredDate = startDate.valueOf()
        }
      } else {
        delete this.change.materialRequiredDate
      }
      ...
      if (this.$refs.form.validate()) {
        ...
        if (this.isCreate) {
          const changeNumber = this.project.nextBuyOutNumber || 0

          this.projectRef.update({
            nextBuyOutNumber: parseInt(changeNumber) + 1
          })
          this.change.createdAt = new Date()

          this.createBuyOut().then(() => {
            this.$snotify.success(
              this.$t('cpm.projects.buyOutCreated'),
          ...
          if (this.change.materialRequiredDateText) {
            this.change.materialRequiredDate =
              this.change.materialRequiredDateText.toString().substring(4, 4) +
              '-' +
              this.change.materialRequiredDateText.toString().substring(0, 2) +
              '-' +
              this.change.materialRequiredDateText.toString().substring(2, 2)
          }
          ...
          this.projectRef
            ...
            .then(() => {
              this.$snotify.success(
                this.$t('cpm.projects.buyOutUpdated'),
                this.$t('alerts.success')
              )
              this.$emit('close', true)
            })
    ...
    setMaterialRequiredDate() {
      this.materialRequiredDateMenu = false
      const dateObj = DateTime.fromMillis(this.change.materialRequiredDate)
      this.change.materialRequiredDateText = dateObj.toFormat('MM/dd/yyyy')
    },
    setPreparedDate() {
