<template>
  <m6-card-dialog
    :title="`${$t('cpm.buyOut')} ${$t('general.beta')}`"
  >
...
import BuyOutCreateModal from '../modals/BuyOutCreateModal'

export default {
  name: 'BuyOut',
  components: {
    'create-modal': BuyOutCreateModal
  },
  ...
  firestore() {
    return {
      buyOuts: this.projectRef.collection('buy_outs').orderBy('number'),
