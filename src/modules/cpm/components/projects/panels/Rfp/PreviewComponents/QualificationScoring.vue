...
export default {
  ...
  data() {
    return {
      headers: [
        { text: ' ', style: 'width:5%;' },
        { text: 'Qualification Scoring', style: 'text-align:center' },
        {
  ...
  computed: {
    scoring() {
      if (!this.proposal.qualificationScoring) {
        return []
      }

      const aux = JSON.parse(JSON.stringify(this.proposal.qualificationScoring))
    ...
    subTotal() {
      let subtotal = 0
      this.proposal.qualificationScoring.map(item => {
